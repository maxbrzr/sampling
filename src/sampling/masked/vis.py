...
def visualize_tasks_overview(
    n_tasks: int,
    x_ctx: Tensor,  # (n_tasks, 1, 1) - context size 1
    y_ctx: Tensor,  # (n_tasks, 1, 1)
    x_tar: Tensor,  # (n_tasks, N_tar, 1)
    y_tar: Tensor,  # (n_tasks, N_tar, 1) - ground truth
    y_pred: Tensor,  # (n_tasks, N_tar, 1) - model predictions
    save_path: str = "tasks_overview.png",
) -> None:
    """
    Visualizes n tasks in a grid format (1 row per task).
    Each row shows the task reconstruction without loss plot.
    Perfect for getting a good overview of model performance across multiple tasks.

    Args:
        n_tasks: Number of tasks to visualize
        x_ctx: Context x with shape (n_tasks, 1, 1)
        y_ctx: Context y with shape (n_tasks, 1, 1)
        x_tar: Target x with shape (n_tasks, N_tar, 1)
        y_tar: Target y (ground truth) with shape (n_tasks, N_tar, 1)
        y_pred: Predicted y with shape (n_tasks, N_tar, 1)
    ...
    # Limit to actual number of tasks
    n_tasks = min(n_tasks, xc.shape[0])

    # Create figure with one row per task
    fig, axes = plt.subplots(n_tasks, 1, figsize=(8, 3 * n_tasks))

    # Handle single task case (axes won't be an array)
    if n_tasks == 1:
        axes = [axes]
    ...
    for i in range(n_tasks):
        ...
        # Context Point (single point for context size 1)
        ax.scatter(
            xc[i], yc[i], c="black", s=60, label="Context (n=1)", zorder=5, marker="o"
        )
        ...
        ax.set_title(f"Task {i + 1}", fontsize=12, fontweight="bold")
    ...
    plt.close()
    print(f"Tasks overview saved to {save_path}")


def create_tasks_from_loader(
    loader: DataLoader,
    encoder: nn.Module,
    decoder: nn.Module,
    device: torch.device,
    ...
    """
    Extracts tasks from a DataLoader and generates predictions.
    Assumes context size 1 for all tasks.

    Args:
        loader: DataLoader yielding (x_full, y_full, x_tar, y_tar) batches
        encoder: Set encoder (MaskedSetEncoder or AdaptiveMaskedSetEncoder)
        decoder: Set decoder (SetDecoder or AdaptiveSetDecoder)
        device: torch device
        n_tasks: Number of tasks to extract

    Returns:
        Tuple of (x_ctx, y_ctx, x_tar, y_tar, y_pred) all with shape (n_tasks, ...)
    """
    encoder.eval()
    decoder.eval()
    ...
    with torch.no_grad():
        for batch_data in loader:
            ...
            # Extract context with size 1
            x_ctx = x_full[:, :1, :]  # (Batch, 1, 1)
            y_ctx = y_full[:, :1, :]  # (Batch, 1, 1)

            # Encode context to get latent representation
            z = encoder(x_ctx, y_ctx, mask=None)  # (Batch, z_dim)

            # Decode to get predictions
            y_pred = decoder(z, x_tar)  # (Batch, N_tar, 1)

            # Add to lists
            for i in range(batch_size):
                x_ctx_list.append(x_ctx[i : i + 1])
                y_ctx_list.append(y_ctx[i : i + 1])
                x_tar_list.append(x_tar[i : i + 1])
                y_tar_list.append(y_tar[i : i + 1])
                y_pred_list.append(y_pred[i : i + 1])

                tasks_collected += 1
                if tasks_collected >= n_tasks:
                    break
    ...
    x_ctx_all = torch.cat(x_ctx_list, dim=0)  # (n_tasks, 1, 1)
    y_ctx_all = torch.cat(y_ctx_list, dim=0)  # (n_tasks, 1, 1)
    x_tar_all = torch.cat(x_tar_list, dim=0)  # (n_tasks, N_tar, 1)
    y_tar_all = torch.cat(y_tar_list, dim=0)  # (n_tasks, N_tar, 1)
    y_pred_all = torch.cat(y_pred_list, dim=0)  # (n_tasks, N_tar, 1)

    return x_ctx_all, y_ctx_all, x_tar_all, y_tar_all, y_pred_all
